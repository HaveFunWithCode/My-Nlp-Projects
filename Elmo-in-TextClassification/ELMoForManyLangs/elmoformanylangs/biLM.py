...
from ELMoForManyLangs.elmoformanylangs.modules.elmo import ElmobiLm
from ELMoForManyLangs.elmoformanylangs.modules.lstm import LstmbiLm
from ELMoForManyLangs.elmoformanylangs.modules.token_embedder import ConvTokenEmbedder, LstmTokenEmbedder
from ELMoForManyLangs.elmoformanylangs.modules.embedding_layer import EmbeddingLayer
from ELMoForManyLangs.elmoformanylangs.modules.classify_layer import SoftmaxLayer, CNNSoftmaxLayer, SampledSoftmaxLayer
from ELMoForManyLangs.elmoformanylangs.dataloader import load_embedding
from ELMoForManyLangs.elmoformanylangs.utils import dict2namedtuple
...
def create_one_batch(x, word2id, char2id, config, oov='<oov>', pad='<pad>', sort=True):
  ...
  if char2id is not None:
    eow_id,bow_id, oov_id, pad_id = char2id.get('<eow>', None), char2id.get('<bow>', None), char2id.get(oov, None), char2id.get(pad, None)
...
def train():
  ...
  if valid_data is None:
    ...
    logging.info(
      "best train ppl: {:.6f}, best valid ppl: {:.6f}, test ppl: {:.6f}.".format(best_train, best_valid, test_result))


def train2():
  args='''
  --train_path data/tweetdb.txt
          --config_path output/fa3/cnn_16_100_512_4096_sample.json
              --model output/fa3/fa_16_100_512_4096_lr0.01_maxcpt20
                --optimizer adam     --lr 0.01     --lr_decay 0.8
                   --max_epoch 10
                      --max_sent_len 15
