...
class Embedder(object):
    ...
    def sent2elmo(self, sent, output_layer=-1):
        read_function = read_sent

        if self.config['token_embedder']['name'].lower() == 'cnn':
            test, text = read_function(sent, self.config['token_embedder']['max_characters_per_token'])
        else:
            test, text = read_function(sent)
