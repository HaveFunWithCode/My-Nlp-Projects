...
def _save_embeddings_parts(X_Word_embeding,
    ...
    store=pd.HDFStore('instaemb_3hmiddle/insta_embedding_{0}.h5'.format(postfix))
...
def load_data(embedding_type='X_avg'):
    ...
    '''
    dfs=[]
    list_hd5s=os.listdir('instaemb_3hmiddle')

    for file in list_hd5s:
        store = pd.HDFStore('instaemb_3hmiddle/'+file)
